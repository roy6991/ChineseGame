...
public class PlayGame4Controller : MonoBehaviour {
	...
	private bool isShowingInstru, isNextRoundConstructed, triggerNextRound, roundStart, isAnsHidden, isSecondClick, isPaused, isSonarPressed,isRoundFail;
	...
	public Button btnContinue, btnSonar;
...
//		int seperatePos = strTargetAudioUrl.LastIndexOfAny(PREFIX_AUDIO_PATH, 0 );
	...
	public void onClickSonar(){
		// allow user to press the sonar when the round is not started 
		if (!roundStart)
			gameStart ();
	}
